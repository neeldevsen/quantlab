...
df = pd.read_csv("Black-76-Call.csv")
...
log_abs_error_BT[:] = np.log(np.abs(binomial_tree[:] - b76_cf_call))

log_abs_error_TT = np.empty(len(N))
log_abs_error_TT[:] = np.log(np.abs(trinomial_tree[:] - b76_cf_call))

log_abs_error_FDM = np.empty(len(N))
log_abs_error_FDM[:] = np.log(np.abs(finite_differences[:] - b76_cf_call))
...
ax.set_title("Convergence Graph for Black-76 Calls")
ax.set_xlabel("log N")
ax.set_ylabel("log Absolute DomainError")
