...
bs_cf_call = bs_cf.put(
    spot, strike, rate, volatility, maturity
)


df_err = pd.read_csv(EXCELS / "Black-Scholes-Put.csv")
df_runtimes = pd.read_csv(EXCELS / "Black-Scholes-Put-Runtimes.csv")
N = np.array(df_err["N"])
...
plt.title("Empirical Computational Complexity for Black Scholes Puts")
