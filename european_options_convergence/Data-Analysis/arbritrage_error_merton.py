...
dfC = pd.read_csv(BASE / "../Excels/Merton-Call.csv")
dfP = pd.read_csv(BASE / "../Excels/Merton-Put.csv")
...
log_arb_error_BT[:] = np.log(np.abs(binomial_tree_call[:] - binomial_tree_put[:] - spot * np.exp(-dividend*maturity) + strike * np.exp(-rate*maturity)))

log_arb_error_TT = np.empty(len(N))
log_arb_error_TT[:] = np.log(np.abs(trinomial_tree_call[:] - trinomial_tree_put[:] - spot * np.exp(-dividend*maturity) + strike * np.exp(-rate*maturity)))

log_arb_error_FDM = np.empty(len(N))
log_arb_error_FDM[:] = np.log(np.abs(finite_differences_call[:] - finite_differences_put[:] -  spot * np.exp(-dividend*maturity) + strike * np.exp(-rate*maturity)))
...
ax.set_title("Arbritrage Error Graph for Merton Options")
ax.set_xlabel("log N")
