...
z = np.polyfit(log_N, log_abs_error_BT, 1)
...
z = np.polyfit(log_N, log_abs_error_TT, 1)
...
z = np.polyfit(log_N, log_abs_error_FDM, 1)
best_line_fit_FDM = np.poly1d(z)
slope_FDM, intercept_FDM = best_line_fit_FDM

plt.scatter(log_N, log_abs_error_BT, color="blue", marker="x")
plt.scatter(log_N, log_abs_error_TT, color="red", marker="x")
plt.scatter(log_N, log_abs_error_FDM, color="black", marker="x")
...
plt.title("Convergence Scatter Plot for Black-Scholes Calls")
plt.xlabel("log N")
