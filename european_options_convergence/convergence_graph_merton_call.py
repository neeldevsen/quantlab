...
df = pd.read_csv("Merton-Call.csv")
...
log_abs_error_BT[:] = np.log(np.abs(binomial_tree[:] - merton_cf_call))

log_abs_error_TT = np.empty(len(N))
log_abs_error_TT[:] = np.log(np.abs(trinomial_tree[:] - merton_cf_call))

log_abs_error_FDM = np.empty(len(N))
log_abs_error_FDM[:] = np.log(np.abs(finite_differences[:] - merton_cf_call))
...
ax.set_title("Convergence Graph for Merton Calls")
ax.set_xlabel("log N")
