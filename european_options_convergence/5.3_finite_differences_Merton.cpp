...
auto generateRHSCall(const OptionDataMerton<S_t, K_t, r_t, q_t, sigma_t, T_t>& data, Coefficients<S_t, K_t, r_t, q_t, sigma_t, T_t>& coeff, auto V, const int n, const int M, const auto deltaT, const auto Smax)
{
    ...
    RHS[M-2] += static_cast<commonType>(0.5) * deltaT * coeff.gammas[M-2] * (Smax * std::exp(-data.dividend * (static_cast<commonType>(n) + static_cast<commonType>(1)) * deltaT) - data.strike * std::exp(-data.rate * (static_cast<commonType>(n) + static_cast<commonType>(1)) * deltaT));
...
PYBIND11_MODULE(merton_fdm, m)
{
    m.doc() = "Merton dividend-adjusted Crank-Nicolson finite difference pricing engine";

    m.def("call",
        [](double spot, double strike, double rate, double dividend, double volatility, double maturity, int M, int N, double multiplier)
        {
